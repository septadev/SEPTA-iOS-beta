...
-(void)loadkml
{
    ...
    NSLog(@"overlays - %lu",(unsigned long)[overlays count]);
...
-(void) getLatestBusJSONData
{
    ...
    if ( [self.travelMode isEqualToString:@"Bus"] )  // Add MFL to this?  Need to investigate this further
    {
        ...
        NSString* stringURL = [NSString stringWithFormat:@"https://www3.septa.org/hackathon/TransitView/%@",itinerary.routeShortName];
...
-(void) addAnnotationsUsingJSONRailData:(NSData*) returnedData
{
    ...
    NSLog(@"DSTVC - addAnntoationsUsingJSONRailData -- added %lu annotations", (unsigned long)[json count]);
...
-(void) addAnnotationsUsingJSONBusData:(NSData*) returnedData
{
    ...
    NSLog(@"DSTVC - addAnntoationsUsingJSONBusData -- added %lu annotations", (unsigned long)[[json objectForKey:@"bus"] count]);
