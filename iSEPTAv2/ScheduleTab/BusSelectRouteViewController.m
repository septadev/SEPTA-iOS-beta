//
//  BusSelectRouteViewController.m
//  iSEPTA
//
...
@interface BusSelectRouteViewController ()
...
@implementation BusSelectRouteViewController
{
...
//    [self.tableView setFrame:CGRectMake(50, 50, 270, 120)];  // 320, 376
...
//        [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:YES];
    ...
    _didJustLoad = NO;
    NSLog(@"BSRVC -(void) viewWillAppear");
    
//    if ( _segueInAction )
//    {
//        [UIView animateWithDuration:0.3f animations:^
//         {
//             [sorterBar setFrame:CGRectMake(0, sorterBar.frame.origin.y, sorterBar.frame.size.width, sorterBar.frame.size.height)];
//         }
//                         completion:^(BOOL finished)
//         {
//             
...
{
    NSLog(@"BSRVC -(void) viewDidAppear");
    
    [super viewDidAppear:animated];
    
//    [self.tableView setBackgroundColor:[UIColor yellowColor]];
//    [self.tableView setFrame:CGRectMake(0, -15, 320, 367+15)];  // 320, 367
...
{
    if ( [_routeData numberOfSections] == 1 )  // Why do this?
        return 0.0f;
    else
        return 22.0f;
...
//                NSLog(@"row: %@", row);
            ...
            case kDisplayedRouteDataRoutes:
                ...
                {
//                    defaultCell = [[BusRoutesDefaultCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:defaultCellID];
//                    nib = [[NSBundle mainBundle] loadNibNamed:@"BusRoutesDefaultCell" owner:self options:nil];
//                    defaultCell = (BusRoutesDefaultCell*)[nib objectAtIndex:0];
...
//    unfilteredList = [[NSMutableArray alloc] init];
    ...
    {
        if ( busFilterStr == nil )
            queryStr = @"SELECT route_short_name, route_id, route_type, route_long_name FROM routesDB WHERE route_type=0 ORDER BY route_short_name ASC";
        else
