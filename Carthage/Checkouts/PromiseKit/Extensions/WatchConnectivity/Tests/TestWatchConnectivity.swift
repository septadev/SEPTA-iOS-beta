...
class WatchConnectivityTests: XCTestCase {
    class MockSession: WCSession {
        ...
        override func sendMessage(_: [String: Any], replyHandler: (([String: Any]) -> Void)?, errorHandler: ((Error) -> Void)?) {
            if fail {
    ...
    func testSuccess() {
        ...
        session.fail = false
        session.sendMessage(["message": "test"]).then { response -> Void in
    ...
    func testFailure() {
        class MockFailSession: WCSession {
            override func sendMessage(_: [String: Any], replyHandler _: (([String: Any]) -> Void)?, errorHandler: ((Error) -> Void)?) {
                errorHandler?(NSError(domain: "Test", code: 1, userInfo: [:]))
            }
        }
        ...
        let session = MockSession.default() as! MockSession
        session.fail = true
        session.sendMessage(["message": "test"]).then { _ -> Void in
