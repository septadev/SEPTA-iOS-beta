...
open class KMLElement {
    ...
    // return All element
    open func findElements<T: KMLElement>(_: T.Type) -> [T]! {
        var elements: [T] = []
        for child: KMLElement in children {
            if let theChild: T = child as? T {
    ...
    open func hasElement<T: KMLElement>(_: T.Type) -> Bool {
        for child: KMLElement in children {
            if child is T {
                return true
            }
...
open class KMLLineStyle: KMLColorStyleGroup, KMLApplyStyle {
    ...
    func applyStyle(_ renderer: MKOverlayPathRenderer) {
        ...
        renderer.lineWidth = CGFloat(width) * 0.5
    }
}
        ...
        if let _styles: [KMLStyleMap] = findElements(KMLStyleMap.self) {
            for style: KMLStyleMap in _styles {
                for (key, value): (String, String) in style.pairs {
                    style.addPairsRef(key, style: styles[value]!)
                }
                styles[style.styleId] = style
            }
        }
    ...
    convenience init(kmlhex: String) {
        ...
        if scanner.scanHexInt64(&hexValue) {
            ...
            case 8:
                ...
                blue = CGFloat((hexValue & 0x00FF_0000) >> 16) / 255.0
                green = CGFloat((hexValue & 0x0000_FF00) >> 8) / 255.0
                red = CGFloat(hexValue & 0x0000_00FF) / 255.0
            default:
