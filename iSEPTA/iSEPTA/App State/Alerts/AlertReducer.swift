...
struct AlertReducer {
    ...
    static func reduceScheduleAction(action: ScheduleAction, state: AlertState) -> AlertState {
        ...
        return AlertState(alertDict: state.alertDict, scheduleState: scheduleState, lastUpdated: state.lastUpdated, alertDetails: state.alertDetails, genericAlertDetails: state.genericAlertDetails)
    }
    ...
    static func reduceNewAlertsRetrieved(action: NewAlertsRetrieved, state: AlertState) -> AlertState {
        return AlertState(alertDict: action.alertsByTransitModeThenRoute, scheduleState: state.scheduleState, lastUpdated: Date(), alertDetails: state.alertDetails, genericAlertDetails: state.genericAlertDetails, appAlertDetails: state.appAlertDetails)
    }

    static func reduceAlertDetailsLoaded(action: AlertDetailsLoaded, state: AlertState) -> AlertState {

        return AlertState(alertDict: state.alertDict, scheduleState: state.scheduleState, lastUpdated: state.lastUpdated, alertDetails: action.alertDetails, genericAlertDetails: state.genericAlertDetails, appAlertDetails: state.appAlertDetails)
    }

    static func reduceGenericAlertDetailsLoaded(action: GenericAlertDetailsLoaded, state: AlertState) -> AlertState {

        return AlertState(alertDict: state.alertDict, scheduleState: state.scheduleState, lastUpdated: state.lastUpdated, alertDetails: state.alertDetails, genericAlertDetails: action.genericAlertDetails, appAlertDetails: state.appAlertDetails)
    }

    static func reduceAppAlertDetailsLoaded(action: AppAlertDetailsLoaded, state: AlertState) -> AlertState {

        return AlertState(alertDict: state.alertDict, scheduleState: state.scheduleState, lastUpdated: state.lastUpdated, alertDetails: state.alertDetails, genericAlertDetails: state.genericAlertDetails, appAlertDetails: action.appAlertDetails)
    }
}
